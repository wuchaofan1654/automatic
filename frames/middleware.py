...
from frames.response import ErrorJsonResponse
from frames.utils.request_util import get_request_ip, get_request_data, get_request_path, get_browser, get_os, \
...
class WebsocketMiddleWare(object):
    ...
    @staticmethod
    def random_str(length=16, timestamp=True):
        str_set = string.ascii_letters + string.digits
        return f"{''.join([random.choice(str_set) for _ in range(length)])} + {timestamp and time.time()}"

    async def __call__(self, scope, receive, send):
        scope['group_name'] = str(hash(f'{self.random_str()}')).replace('-', 'A')
