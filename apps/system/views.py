...
from apps.system.models import LoginInfo, OperationLog, CeleryLog
...
from frames.utils.export_excel import export_excel_save_model
from frames.utils.file_util import get_all_files, remove_empty_dir, delete_files
from frames.utils.system_info_utils import get_memory_used_percent, get_cpu_used_percent, get_disk_used_percent
...
class GetLocalIp(APIView):
    @staticmethod
    def get(request):
        pattern = re.compile(pattern=r'inet.*(192.168.\d+.\d+).*netmask')
        output = subprocess.Popen(f"ifconfig", stdout=subprocess.PIPE, shell=True)
        while True:
            data = output.stdout.readline().decode('utf-8')
            local_ip = None
            if data:
                result = pattern.findall(data.strip())
                if result:
                    local_ip = result[0]
                    return SuccessResponse(local_ip)
            else:
                break
