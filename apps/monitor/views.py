...
from apps.monitor.filters import ServerFilter, MonitorFilter
from apps.monitor.models import Server, Monitor, SysFiles
from apps.monitor.serializers import ServerSerializer, MonitorSerializer, UpdateServerSerializer
...
class MonitorModelViewSet(CustomModelViewSet):
    ...
    def enabled_monitor_info(self, request: Request, *args, **kwargs):
        ...
        # 定时获取系统监控信息
        periodictask_obj = PeriodicTask.objects.filter(task='apps.monitor.tasks.get_monitor_info').first()
        if not periodictask_obj:
            ...
            periodictask_obj.task = "apps.monitor.tasks.get_monitor_info"
        ...
        clean_task_obj = PeriodicTask.objects.filter(
            task='apps.monitor.tasks.clean_surplus_monitor_info').first()
        if not clean_task_obj:
            ...
            clean_task_obj.task = "apps.monitor.tasks.clean_surplus_monitor_info"
